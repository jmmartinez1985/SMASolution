...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			//IEM - AGREGADO PLACEHOLDER
			if (!$.isEmptyObject(that.options.placeHolder)) {
			    debugger;
			    $("#" + that.id).css("background-image", 'url('+that.options.placeHolder+')');
			}
			else { //PLACE HOLDER DEFAULT
			    $("#" + that.id).css("background-image", 'url(../Images/No_Profile.jpg)');
			}
		},
		initDestroy: function () { //initDestroy PARA LOS CASOS EN QUE SE AGREGA UNA IMAGEN DEFAULT PERO SE DESEA ELIMINAR POSTERIORMENTE
		    debugger;
		    var that = this;
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                //CAMBIO A INIT DESTROY PARA QUE ELIMINE EL PLACE HOLDER 
                that.initDestroy();
